"""
Модуль для конвертации чисел из римской системы счисления в арабскую и обратно.
"""
...
def roman_to_int(roman_digit: str) -> int:
    ...
    Аргументы:
        roman_digit (str): Строка с римским числом.

    Возвращает:
        int: Число в арабской системе счисления.
    """
    result = 0
    ...
    for index in range(len(roman_int_digits) - 1):  # тут суммируем все значения с учетом правил
        if roman_int_digits[index] >= roman_int_digits[index + 1]:  # если справа старшая степень - вычитаем курентное из резалта,
            result += roman_int_digits[index]                      # в противном случае наоборот
        else:
...
def int_to_roman(int_digit: int) -> str:
    ...
    Аргументы:
        int_digit (int): Число в арабской системе счисления.

    Возвращает:
        str: Строка с числом в римской системе.
    """
    result = []
